...
function getUniformCriteria(student, classDoc) {
  ...
  const name = classDoc.name.toLowerCase().replace(/-/g, ' ');
  if (name.includes('pra paud')) return false;
...
async function processBranch(branch, activeAcademicYear) {
  ...
  for (const sb of sortedSubBranches) {
    ...
    for (const entry of entries) {
      const ukuran = getUniformCriteria(entry.student, entry.classDoc) ? '' : 'Tidak memenuhi kriteria';
      entry.ukuran = ukuran;
    ...
    for (const entry of normal) {
      ...
      const row = ws.addRow([no, student.name || '', student.nis || '', kelas, dateValue, null, '']);
      if (dateValue) {
        row.getCell(5).numFmt = 'dd/mm/yyyy';
        row.getCell(6).value = { formula: `DATEDIF(E${row.number},TODAY(),"Y")` };
      }
      no++;
    }
    ...
    if (flagged.length > 0) {
      ...
      for (const entry of flagged) {
        ...
        const row = ws.addRow([no, student.name || '', student.nis || '', kelas, dateValue, null, 'Tidak memenuhi kriteria']);
